...
class FaqEngine:
    ...
    def query(self, usr):
        try:
            ...
            if t_usr_array is None or (t_usr_array == 0).all():
                response_data = {
...
def query_faq():
    ...
    if not faqmodel:
        ...
        faqslist = [os.path.join(base_path, "Greetings.csv"), os.path.join(base_path, "questions.csv")]
    ...
    # Check if the input is in the format "Subject: Description"
    if ":" in user_input:
        try:
            subject, description = map(str.strip, user_input.split(":", 1))
            # print(subject + description)
            resp = requests.post("https://b620-2401-4900-1f27-6f9b-b09a-e8a5-9d03-52ef.ngrok-free.app/createTicket",
                                 json={
                                     "subject": subject,
                                     "description": description,
                                     "to": "imthiyas.vadakkan@tarento.com",
                                     "sender": "varsha.mahuli@tarento.com",
                                     "id": "18296"
                                 })
            response_data1 = resp.json()
            print("Output from Ticket creation:", response_data1)

            if response_data1 and "message" in response_data1:
                response = [{
                    "text": response_data1["payload"],
                    "Context": "NewTicket",
                    "Confidence": 1.0,
                    "type": {"type": 'direct',
                             "text": response_data1["payload"]}
                ...
                response = {"message": "Empty or invalid response received from the ticket creation API."}

        except Exception as e:
            print("Error while creating ticket:", e)
            response = {"message": "Error while creating ticket"}

        return jsonify(response)

    # Check for other types of questions
    if hitl_flag:
        if user_input.lower() == "Thank you Agent":
            hitl_flag = False
            response = [{"text": "Exited HITL mode.", "Context": "HITL", "Confidence": 1.0,
                         "type": {"type": 'direct', "text": "Exited HITL mode."}}]
        else:
            try:
                print("user input is " +user_input )
                resp = requests.post("https://1b2c-59-92-73-255.ngrok-free.app/hitl/message",
                                     json={"message": user_input})
                response_data1 = resp.json()
                print("Response from HITL endpoint:", response_data1)

                if response_data1 and "message" in response_data1:
                    response = [{
                        "text": response_data1["message"],
                        "Context": "HITL",
                        "Confidence": 1.0,
                        "type": {"type": 'direct',
                                 "text": response_data1["message"]}
                    }]
                    # response = [{"message": "hi from hitl."}]

                else:
                    response = {"message": "Empty or invalid response received from the HITL API."}

            except Exception as e:
                print("Error while sending question to HITL:", e)
                response = {"message": "Error while sending question to HITL"}
    else:
        response_data = faqmodel.query(user_input)
        print("Class:", response_data)
        ...
        if response_data["Context"] == "hitl" or response_data["Context"] == "PlaceOrder":
            hitl_flag = True

        elif response_data["Context"] == "CancelOrder" or response_data["Context"] == "CheckOrderStatus":
            try:
                data = request.get_json()
                user_question = data["Question"]
                resp = requests.post("https://008b-115-99-95-47.ngrok-free.app/process_email", json={
                    "input": "FROM_USER " + data["Question"],
                    "conversationID": 1,
                    "mailID": "varsha.mahuli@tarento.com"
                })
                response_data1 = resp.json()
                print("Output from Order tracking Rithu : ", response_data1)

                if response_data1 and isinstance(response_data1, list) and len(response_data1) > 0:
                    response_data1 = response_data1[0]  # Get the first element of the list

                    if "answer" in response_data1:
                        response = [{
                            "text": response_data1["answer"],
                            "Context": response_data["Context"],
                            "Confidence": 1.0,
                            "type": {"type": 'direct',
                                     "text": response_data1["answer"]}
                        ...
                        response = {"message": "Invalid response received from Rithu's API."}
                else:
                    response = {"message": "Empty or invalid response received from Rithu's API."}

            except Exception as e:
                print("Error while processing order tracking:", e)
                response = {"message": "Error while processing order tracking"}
